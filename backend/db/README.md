...
### DynamoDB schema
- Partition key: `id` (a UUID). `question_id` is also stored for legacy compatibility, but lookups use `id`.
- Sort key: `question_topic` is required in production; new questions default to `General` when omitted.
...
- Single-question reads use a `query` on the `id` partition key to support tables with a `question_topic` sort key.
